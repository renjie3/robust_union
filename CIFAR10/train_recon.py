...
lr_schedule_func = lambda t: np.interp([t], [0, epochs*2//5, epochs*4//5, epochs], [0.005, 0.1, 0.005, 0])[0]
...
attack_list = {"pgd_linf": pgd_linf,  "pgd_l1_topk": pgd_l1_topk, "pgd_l2": pgd_l2, "msd_v0": msd_v0, "triple_adv": triple_adv, "pgd_worst_dir": pgd_worst_dir, "vanilla": triple_adv, "pgd_l1_sign_free": pgd_l1_sign_free}
...
for epoch_i in range(1,epochs+1):
    ...
    if choice == "vanilla":
        ...
        train_loss, train_acc = triple_adv_recon(train_loader, model, epoch_i, attack, criterion, opt = opt, device = device, epsilon_l_2 = 0.3)
    elif choice == "msd_v0":
        train_loss, train_acc = epoch_adversarial_recon(train_loader, model, epoch_i, attack, criterion, opt = opt, device = device, epsilon_l_2 = 0.3)
    elif choice == "pgd_worst_dir":
        train_loss, train_acc = epoch_adversarial_recon(train_loader, model, epoch_i, attack, criterion, opt = opt, device = device, epsilon_l_2 = 0.3, alpha_l_inf = 0.005)
    elif choice == "pgd_l1_sign_free":
        train_loss, train_acc = epoch_adversarial_recon(train_loader, model, epoch_i, attack, criterion, opt = opt, device = device, alpha = params.alpha, num_iter = params.num_iter)
    else:
        train_loss, train_acc = epoch_adversarial_recon(train_loader, model, epoch_i, attack, criterion, opt = opt, device = device, alpha = params.alpha, num_iter = params.num_iter)
    lr_schedule.step()
